...
export async function saveMISData(data: MISStorageData): Promise<boolean> {
  try {
    ...
    // Strip bulky transaction data to avoid localStorage quota issues
    // These can be regenerated by re-uploading source files
    const compactData: MISStorageData = {
      ...data,
      periods: data.periods.map(record => stripBulkyData(record))
    };
    ...
    // If still failing, try clearing old data first
    if (error instanceof DOMException && error.name === 'QuotaExceededError') {
      ...
      try {
        ...
        const compactData: MISStorageData = {
          ...data,
          periods: data.periods.slice(0, 6).map(record => stripBulkyData(record))
        };
        localStorage.setItem(STORAGE_KEY, JSON.stringify(compactData));
        localStorage.setItem(PATTERNS_KEY, JSON.stringify(data.learnedPatterns));
        console.log('Saved with reduced data (last 6 months only)');
        return true;
      } catch (retryError) {
        console.error('Still failing after cleanup:', retryError);
      }
    }
...
/**
 * Clear all MIS data from localStorage
 * Use this to recover from quota exceeded errors
 */
export function clearAllMISData(): void {
  ...
  console.log('All MIS data cleared from localStorage');
}
...
export function getStorageInfo(): { used: number; available: number; percentage: number } {
  ...
  // localStorage typically has 5-10 MB limit, assume 5 MB
