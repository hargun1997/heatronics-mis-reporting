...
function persistQueue(q: QueueItem[]) {
  try {
    ...
    // Quota exceeded — queue still works in memory until refresh.
  }
}
...
// --------------------------------------------------------------------------
// Queue card — list of bookings saved in the current session
// --------------------------------------------------------------------------
    ...
    <div className="rounded-xl border border-emerald-300 bg-white">
      <div className="px-4 sm:px-5 py-3 border-b border-slate-100 flex items-baseline justify-between gap-2">
        <div>
          ...
          <div className="text-xs text-slate-500 mt-0.5">
            {queue.length} booking{queue.length === 1 ? '' : 's'} · saved in this browser
          </div>
        </div>
        {!confirmClear ? (
          <button
      ...
      <ul className="divide-y divide-slate-100">
        {queue.map((q, i) => {
          ...
          const titleLine =
            q.advice.invoiceExtract?.vendor ||
            q.answers.party ||
            q.manualEntry?.description ||
            'Unnamed expense';
          return (
            <li key={q.id}>
              <button
